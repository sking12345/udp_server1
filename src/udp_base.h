...
class UdpBase {
  ...
  public:
	...
	uint32 save_addr(uint32 uid, struct sockaddr_in addr, uint8 type = _UDP_TYPE_PSP_);	//保存外网信息
	struct extranet_addr* get_addrs(uint32); //获取外网信息
	uint32 get_socket_addrs(uint32); //向服务器获取外网信息

	struct udp_pack* get_missing_pack(struct udp_pack);	//获取确实的包,用于补包
	void set_verify_recv_log(uint8 type);	//设置是否需要验证接受数据的记录
